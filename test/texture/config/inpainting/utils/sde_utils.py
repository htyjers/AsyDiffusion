...
class IRSDE(SDE):
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state',GT = None, mask = None, S_sde = None, S_GT = None, S_LQ = None, dis = None, S_LQs = None, **kwargs):
        ...
        for t in tqdm(reversed(range(1, int(T+1)))):# Early Stage

            g = 1
            if t % g == 0:
                xs_optimum = S_sde.generate_states(x0=S_GT.cuda() * mask.cuda(), mu=S_LQs.cuda() * mask.cuda(), timesteps = t-g)
            ...
            g = 1
            if t % g == 0:
                xs_optimum1 = self.generate_random_states_texture(x0=GT.cuda() * mask.cuda(),
                                                                  mu=GT.cuda() * mask.cuda(), timesteps=t-g)
            x_original = xs_optimum1 * mask.cuda() + x_original * (1 - mask.cuda())
