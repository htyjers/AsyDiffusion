...
def main():
    ...
    for epoch in range(0, 1):
        ...
        for g, train_data in enumerate(test_loader):
            ...
            myinput = Y_GT * mask
            myinput1 = torch.cat([X_GT * mask, X_GT * mask, X_GT * mask],1)
            noisy_state = sde.noise_state(myinput)
            noisy_states = S_sde.noise_state(myinput1)
            model.feed_data(noisy_state, myinput, Y_GT, mask, S_sde, Y_GT,  myinput1)# X_GT #X_LQ
            tic = time.time()
            model.test(sde, save_states=True, save_dir=dataset_dir, GT = Y_GT, mask = mask, S_sde = S_sde, S_GT = Y_GT, S_LQ = noisy_states, dis = model.dis)
