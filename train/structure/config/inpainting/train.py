...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for gg, train_data in enumerate(train_loader):
            ...
            Y_GT, X_GT, _ = train_data["GT"], train_data["GT_gray"], train_data["GT_edge"]
            ...
            timesteps, states = sde.generate_random_states(x0=Y_GT,mu=torch.cat([X_GT * mask, X_GT * mask, X_GT * mask],1))
            model.feed_data(states, torch.cat([X_GT * mask, X_GT * mask, X_GT * mask], 1), Y_GT, mask, S_sde, Y_GT,
                            torch.cat([X_GT, X_GT, X_GT], 1))
